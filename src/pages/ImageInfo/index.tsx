...
class Index extends Component<any, any>{
  constructor(prop: any) {
    ...
    this.state.imageUri = this.props.route.params.imageInfo.content
  }
  getArtilceInfo = async (article_id: any = this.state.imageDate.article_id) => {
    ...
    if (res.status == 200) {
      ...
      this.setState({ imageDate: { ...res.article[0] }, comment2Date: comment[0] })
    }
  }
  commentinput: any
  state = {
  ...
  // 关注
  attention = async () => {
    const res = await request.post('/attention', { uid: this.props.userInfo.uid, aid: this.state.imageDate.uid })
    if (res.status == 200) {
      Alert.alert('关注用户成功')
    } else {
      Alert.alert('已关注')
  ...
  hideAlert = async (isdelete?: Boolean) => {
    const { visible, imageDate, deletecomment_id, comment2Date } = this.state

    if (visible) {
      if (isdelete) {
        const res = await request.delete(`/comment2?comment2_id=${deletecomment_id}`)
        if (res.status == 200) {
          this.getArtilceInfo(imageDate.article_id)
          this.setState({
            showAlert2: false,
          });
        }
  ...
  // 提交评论
  postComment = async () => {
    const { commentValue, imageDate, comment2placeholder, comment2user, visible, comment2Date } = this.state
    if (visible) {
      const res = await request.post('/comment2', { uid: this.props.userInfo.uid, aid: comment2Date.uid, article_id: imageDate.article_id, text: commentValue, comment_id: comment2Date.comment_id })
      if (res.status == 200) {
        Alert.alert('评论成功')
      } else {
        Alert.alert('评论失败')
      ...
      if (comment2user) {
        const res = await request.post('/comment2', { uid: this.props.userInfo.uid, aid: comment2user.uid, article_id: imageDate.article_id, text: commentValue, comment_id: comment2user.comment_id })
        if (res.status == 200) {
          Alert.alert('评论成功')
        } else {
          Alert.alert('评论失败')
        ...
        if (commentValue.trim().length > 0) {
          const res = await request.post('/comment', { uid: this.props.userInfo.uid, aid: imageDate.uid, article_id: imageDate.article_id, text: commentValue })
          if (res.status == 200) {
            Alert.alert('评论成功')
          } else {
            Alert.alert('评论失败')
  ...
  render() {
    const {comment2placeholder,commentValue, showAlert2,showAlert,visible, imageDate, imageUri ,comment2Date} = this.state
    return (
      <View style={{ flex: 1, position: "relative", backgroundColor: "#fff" }}>
        <GobackTitle title='校园论坛' props={this.props} />
        <ImageView imageArr={imageUri} />
        {/* 昵称开始 */}
        {
          !visible ?
            <View style={{ flex: 1 }}>
              <View style={{ paddingLeft: 15, paddingRight: 15 }}>
                <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", height: 70 }}>
                  <TouchableOpacity onPress={()=>this.gotoUserDetail(imageDate.uid)} style={{ flexDirection: "row", alignItems: "center" }}>
                    ...
                    <View style={{ marginLeft: 10 }}>
                      ...
                      <Text style={{ marginTop: 3, color: "#666" }}>{lastTimeStr(imageDate.create_time)}</Text>
                    </View>
                  </TouchableOpacity>
                  {
                    this.props.userInfo.uid != imageDate.uid ?
                ...
                <View style={{ paddingLeft: 15, paddingBottom: 10 }}>
                  <View style={{ flexDirection: "row" }}>
                    <TouchableOpacity style={styles.videoType}>
                      ...
                      <Text style={{ marginLeft: 5 }}>{imageDate.article_type}</Text>
                    </TouchableOpacity>
...
const ImageInfo = connect(state => ({ userInfo: state.userInfo }), {})(Index)
