  ...
  function transitionEmoji (text:any){
    const conent =  regularEmoji(text)
     ...
     if(v.text=='\n'){
      ...
      if(v.image){
       return <Image style={{width:25,height:25,...props.emojiStyle}} source={EMOTIONS_DATA[v.image]} />
      }else if(v.text){
       return <Text style={{fontSize:14,color:'#666',...props.fontStyle}}>{v.text}</Text>
      }
    })
   }
  return (
